...
{
    public class EntityInfoWindow : PulsarGuiWindow
    {
        private ComponentInstance[][] _componentInstances = new ComponentInstance[0][];
        private Entity? _selectedEntity;

	    private EntityInfoWindow()
	    {
	        _flags =  ImGuiWindowFlags.NoCollapse;// | ImGuiWindowFlags.AlwaysAutoResize;
        }

        internal static EntityInfoWindow GetInstance() {

            EntityInfoWindow thisItem;
            if (!_uiState.LoadedWindows.ContainsKey(typeof(EntityInfoWindow)))
            {
                thisItem = new EntityInfoWindow();
            }
            else
            {
                thisItem = (EntityInfoWindow)_uiState.LoadedWindows[typeof(EntityInfoWindow)];
            }
