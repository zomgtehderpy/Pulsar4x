...
namespace Pulsar4X.ImGuiNetUI.EntityManagement
{
    public class OrderCreationWindow : PulsarGuiWindow
    {
        ...
        private OrderCreationWindow()
        {
            _flags = ImGuiWindowFlags.None;
        }

        public static OrderCreationWindow GetInstance()
        {
            OrderCreationWindow thisItem;
            if (!_uiState.LoadedWindows.ContainsKey(typeof(OrderCreationWindow)))
            {
                thisItem = new OrderCreationWindow();
            }
            else
            {
                thisItem = (OrderCreationWindow)_uiState.LoadedWindows[typeof(OrderCreationWindow)];
            }
        ...
        internal override void Display()
        {
            ...
            if (IsActive && ImGui.Begin(WindowTitleHelper.GetDebugWindowTitle("Order Creation"), ref IsActive, _flags))
            {
                RenderTabOptions();
