...
namespace Pulsar4X.SDL2UI
{
    public class OrdnanceDesignWindow : PulsarGuiWindow
    {
        ...
        private OrdnanceDesignWindow()
        {
            HardRefresh();
        }

        public static OrdnanceDesignWindow GetInstance()
        {
            OrdnanceDesignWindow thisitem;
            if (!_uiState.LoadedWindows.ContainsKey(typeof(OrdnanceDesignWindow)))
            {
                thisitem = new OrdnanceDesignWindow();
            }
            else
                thisitem = (OrdnanceDesignWindow)_uiState.LoadedWindows[typeof(OrdnanceDesignWindow)];
        ...
        {
            if(_currentDesigns == null || _faction == null)
                return;
